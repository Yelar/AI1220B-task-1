...
**Team Size:** 3 members  
**PoC Stack for This Submission:** Next.js frontend, FastAPI backend, local SQLite database, LM Studio local LLM API with mock fallback for testing

### Current PoC Status for This Submission

This report contains two kinds of content:

1. **Target-system requirements and architecture** for the collaborative editor the team plans to build over the semester.
2. **Current proof-of-concept status** showing what is already implemented in code for Assignment 1.

Implemented in the current submission:
- working Next.js dashboard and editor pages
- frontend-to-backend document create, load, list, update, delete, export, and version flows
- local demo users and backend-enforced document permissions
- AI invocation and history flows through `/api/ai/invoke` and `/api/ai/history`
- basic WebSocket collaboration room with structured presence and document-update events
- role-aware UI states in the frontend with local role switching for demo coverage

Planned for later milestones rather than fully implemented now:
- full conflict-safe collaborative editing
...
|---|---|---|
| **Frontend App** | Document pages, editor shell, AI panel, presence UI, and permission-aware interactions | Next.js, React, TypeScript |
...
| Method | Endpoint | Request Body | Response | Errors |
|---|---|---|---|---|
| POST | `/api/ai/invoke` | `{ feature, selected_text, surrounding_context?, target_language?, document_id? }` | `200 { feature, output_text, model_name, provider, status, mocked }` | 403, 404, 502 |
...
- the backend already exposes local demo users through header-based identity selection
- document permissions are enforced on backend routes and reflected in the frontend workspace
- the current PoC does not include password login, OAuth, or long-lived auth sessions
...
In this PoC design, the frontend sends a regular HTTP request to `/api/ai/invoke` and shows a loading state until the local model returns. This is acceptable for the current implementation because LM Studio runs on the same machine and the goal is a low-complexity local proof of concept.
...
This is a simple single-repository submission. The goal is clarity, not tooling sophistication. The repository root contains both application folders and the assignment documents, so evaluators can see the code and the written deliverables in one place.
...
### Testing Structure
- **Backend tests:** route and service tests under `backend/tests/` covering documents, permissions, versions, AI flows, and collaboration behavior.
- **Frontend automated tests:** not included yet in the current submission.
- **Integration coverage:** backend tests use a temporary SQLite database and mocked LM Studio when needed.
- **End-to-end coverage:** the final PoC walkthrough is validated through manual frontend-backend demo flows.
...
The PoC stores document content as text in SQLite. This keeps the current implementation easy to inspect and debug. If the collaboration layer later adopts CRDT snapshots, the content field can evolve to structured JSON without breaking the high-level model.
...
| **Decision** | Keep both apps in one repository root under `frontend/` and `backend/`, alongside the assignment documents. |
...
|---|---|---|
| **Member A** | Frontend & Editor UX | Next.js pages, dashboard, editor shell, AI panel, presence UI, and API integration from the frontend |
...
| **M2: Testing and Stabilization** | 2026-04-01 | Completed. Backend tests passed, API behavior was validated, and integration issues were reviewed before demo preparation. |
...
## 4.1 Current PoC Scope

The current repository demonstrates the architectural skeleton required by the assignment rather than a feature-complete product.

Implemented in code today:
- a working frontend dashboard that lists documents and creates new ones
- a document editor page that loads and updates document content through the backend API
- AI assistant controls that send selected text to the backend for rewrite, summarize, translate, and restructure requests
- version list, snapshot creation, and owner-only revert controls
- local demo users, permissions management, and export actions
- a collaboration room connection with presence sync, reconnect state, and structured live update events

Intentionally not complete yet:
- real authentication screens or login flow
- cursor-level collaborative editing or CRDT-based conflict resolution
- comprehensive frontend automated tests

## 4.2 Frontend-to-Backend Communication

The PoC satisfies the core integration requirement:

- `POST /api/documents` is used from the dashboard to create a document
- `GET /api/documents/:id` is used to load the editor
- `PATCH /api/documents/:id` is used to save document changes
- `POST /api/documents/:id/versions` is used to create version snapshots
- `POST /api/documents/:id/versions/:version_id/revert` is used to restore an older version
- `GET /api/documents/:id/permissions` and `POST /api/documents/:id/permissions` are used to manage sharing
- `POST /api/ai/invoke` is used to request AI suggestions
- `GET /api/users` and `GET /api/users/me` are used to load local demo identities
- `WS /ws/documents/:id` is used for live room state and update broadcasts

This proves that the frontend exists, loads, and communicates with the backend using the contract described in Part 2.

## 4.3 Data Contract Validation

The current frontend code expects and uses the same document shape described in the API section:
...
```json
{
  ...
  "created_at": "2026-04-01T10:20:27",
  "updated_at": "2026-04-01T10:20:27"
}
```

The AI request/response flow also matches the current backend contract:
- request: `feature`, `selected_text`, `surrounding_context`, optional `target_language`, optional `document_id`
- response: `feature`, `output_text`, `model_name`, `provider`, `status`, `mocked`

## 4.4 Repository and README Fit

The repository is organized around two application folders:
...
For Assignment 1, the PoC goal is satisfied: the frontend communicates with the backend, the main data contracts are visible in working code, the repository structure is understandable, and the setup remains local-first and reproducible.
